...
class ModelLanguage(): # 语音模型类
    ...
    def SpeechToText(self, list_syllable, syllableDecodeNum=10):
        '''
        语音识别专用的处理函数
        ...
        while len(tmp_list_syllable) > 0:
            # 拼音序列过长最多尝试解码syllableDecodeNum个拼音
            if syllableDecodeNum < len(tmp_list_syllable):
                lst_syllable_remain = tmp_list_syllable[syllableDecodeNum:]
                tmp_list_syllable = tmp_list_syllable[:syllableDecodeNum]
