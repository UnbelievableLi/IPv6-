...
class ModelSpeech:
    ...
    def recognize_speech_from_file(self, filename):
        ...
        wavSignalNumber = (int(len(wavsignal[0]) / 16 - 25) // 10 + 1) // 1600 + 1
        wavSignalLength = len(wavsignal[0]) // ((int(len(wavsignal[0]) / 16 - 25) // 10 + 1) // 1600 + 1) + 1
        r = []
        for i in range(1, wavSignalNumber):
            r.extend(self.recognize_speech(wavsignal[0][(i - 1) * wavSignalLength : i * wavSignalLength].reshape(1, wavSignalLength), sample_rate))
        if (wavSignalNumber - 1) * wavSignalLength < len(wavsignal[0]):
            r.extend(self.recognize_speech(wavsignal[0][(wavSignalNumber - 1) * wavSignalLength : len(wavsignal[0])].reshape(1, len(wavsignal[0]) - (wavSignalNumber - 1) * wavSignalLength), sample_rate))
