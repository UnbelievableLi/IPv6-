...
class Diarizer:
    ...
    def recording_embeds(self, signal, fs, speech_ts):
        ...
        for utt in tqdm(speech_ts, desc='Extracting embeddings', position=0):
    ...
    def diarize(self,
        ...
        Diarize a 16khz mono wav file, produces list of segments
            ...
            Inputs:
                ...
                num_speakers_ult (int): Number of speakers >= num_speakers, <= num_speakers_ult
        ...
        cluster_labels, cluster_score = self.cluster(embeds, n_clusters=num_speakers,
                                                      threshold=threshold, enhance_sim=enhance_sim)
        for num_speakers_i in range(num_speakers + 1, num_speakers_ult + 1):
            print('Clustering to {} speakers...'.format(num_speakers_i))
            cluster_labels_i, cluster_score_i = self.cluster(embeds, n_clusters=num_speakers_i,
                                                                threshold=threshold, enhance_sim=enhance_sim)
            if cluster_score < cluster_score_i:
                cluster_labels = cluster_labels_i
                cluster_score = cluster_score_i
        print('Clustered to {} speakers.'.format(len(set(cluster_labels))))
