...
from . import main
from ..models import Status, Info,Ping,Ping1,School
...
def index():
    ...
    if form.submit.data and form.validate():
        ...
        return redirect(url_for('main.search1', kw=kt))
         ...
         pagination = Status.query.paginate(page, per_page=current_app.config['NUMBER_PER_PAGE'], error_out=False)
         result = pagination.items
         Count = Info.query.count()
         Count1 = Status.query.filter(or_((Status.http2_v6=='Y'),(Status.http_v6=='Y'),(Status.https_v6=='Y'))).count()
         Cout_7=Ping1.query.all()
         for t in Cout_7:
...
def search(kw):
    ...
    if form.submit.data and form.validate():
        ...
        return redirect(url_for('main.search1', kw=kt))
         ...
         Count= Info.query.filter(Info.province.like(kw)).count()
         result1= Ping.query.filter_by(province=kw).first()
         Count1=result1.count #4月份数据
         y=Ping1.query.filter_by(province=kw).first()
    ...
    #   if ((r.url_info.http_v6=='Y') | (r.url_info.https_v6=='Y') | (r.url_info.http2_v6=='Y')):
     #       Count1=Count1+1
        ...
        #if ((r.url_info.http_v6=='Y') | (r.url_info.https_v6=='Y') | (r.url_info.http2_v6=='Y')):
         #   Count1=Count1+1
         return render_template('search.html', form=form,form1=form1, content=all_info, pagination=pagination,kw=kw,Count=Count,Count1=Count1,Count2=Count2,
